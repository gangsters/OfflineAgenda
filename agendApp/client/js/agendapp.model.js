agendapp.model = {
	 ...
	 */
	 launch_actualization: function() {
	 	console.log(">> launch_actualization");
	 	if(IS_ONLINE){
	 		agendapp.model.get_all_events_from_server(agendapp.model.callback_get_from_server_actualization);
	 	}
	 },
		...
		for (var i = 0; i <locallength; i++) {
			...
			if ((agendapp.model.events_local[i].isDirty != 'undefined') && agendapp.model.events_local[i].isDirty) {
				...
				// events to create or update
					...
					var that = agendapp.model.events_local[i];
					console.log("that = ");
					console.log(that);
